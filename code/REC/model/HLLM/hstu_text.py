...
class HSTUwithText(BaseModel):
    ...
    def __init__(self, config, dataload):
        ...
        if self.id_emb:
            self.weights = nn.Parameter(torch.randn(1).float(), requires_grad=True)
            self.concatLR = nn.Linear(4096, 2048)
    ...
    def forward_item_emb(
        ...
        # print(llm.get_input_embeddings().weight)
        ...
        if not self.id_emb:
            ...
            id_emb = self.user_llm.get_input_embeddings()(item_ids)
            if id_emb.shape != emb.shape:
                id_emb = id_emb.flatten(0, 1)
            if id_emb.shape != emb.shape:
                print("emb.shape", id_emb.shape, emb.shape)
            if self.id_emb == "only_id_emb":
                return id_emb
            elif self.id_emb == "concat":
                concated_emb = torch.concat([emb, id_emb], dim = -1)
                return self.concatLR(concated_emb)
            elif self.id_emb == "mean":
                return (id_emb + emb)/2
            elif self.id_emb == "weighted_avg":
                normalized_weights = F.sigmoid(self.weights)
    ...
    def forward(self, interaction, mode='train'):
        ...
        pos_embedding = self.forward_item_emb(pos_input_ids, pos_position_ids, pos_cu_input_lens, self.item_emb_token_n, self.item_emb_tokens, self.item_llm, pos_item_ids)
        pos_embedding = pos_embedding.reshape(N, S+1, -1)  # [batch_size * seq_len, 2048] -> [batch_size, seq_len, 2048]
        neg_embedding = self.forward_item_emb(neg_input_ids, neg_position_ids, neg_cu_input_lens, self.item_emb_token_n, self.item_emb_tokens, self.item_llm, neg_item_ids)
