...
class HLLM(BaseModel):
    ...
    def __init__(self, config, dataload):
        ...
        if self.id_emb != "text":
            ...
            self.ffn = nn.Linear(4096, 2048)
            self.ffn_cold = nn.Linear(4096, 2048)
            self.ffn_warm = nn.Linear(4096, 2048)
            self.ffn_for_ca = nn.Sequential(
                nn.Linear(2048, 2048),
                nn.ReLU(),
                nn.Linear(2048, 2048),
            )
            self.weight_emb = nn.Parameter(torch.randn(2048), requires_grad=True)
            self.item_num = dataload.item_num
            self.attention_layer1 = nn.MultiheadAttention(embed_dim=2048, num_heads=16, dropout=0.1, batch_first=True)
            self.attention_layer2 = nn.MultiheadAttention(embed_dim=2048, num_heads=16, dropout=0.1, batch_first=True)
            self.attention_layer3 = nn.MultiheadAttention(embed_dim=4096, num_heads=32, dropout=0.1, batch_first=True)
            self.layernorm3 = nn.LayerNorm(2048) 
            self.layernorm4 = nn.LayerNorm(2048) 
        self.layernorm1 = nn.LayerNorm(2048) 
        self.layernorm2 = nn.LayerNorm(2048) 
        if config['lr_mult_prefix']:
            print("Using hierarchical lr")
            self.hierarchical_lr = True
            self.item_embedding = nn.Embedding(self.item_num, 2048, padding_idx=0)
    ...
    def emb_fusion(self, item_ids, emb, is_cold=None):
        ...
        if self.id_emb == "id":
            return id_emb
        if not is_cold or self.by_case is None or self.by_case == False:
            B = id_emb.shape[0]
        ...
        if self.id_emb == "concat":
            ...
            g_input = torch.cat([emb, id_emb], dim=-1)  
            # 线性变换 + sigmoid 得到门控向量
            g = torch.sigmoid(self.ffn(g_input))    
            # 融合: g * user_emb + (1 - g) * item_emb
            fused_emb = g * emb + (1.0 - g) * id_emb
            return fused_emb
        elif self.id_emb == "domain_gating":
            # 1
            # item_weight = torch.exp(torch.sum(self.weight_emb * id_emb, dim=-1, keepdim=True).clamp(max=10))
            # text_weight = torch.exp(torch.sum(self.weight_emb * emb, dim=-1, keepdim=True).clamp(max=10))
            
            # total_weight = item_weight + text_weight + 1e-8

            # item_weight = item_weight / total_weight
            # text_weight = text_weight / total_weight

            # # 加权融合
            # weighted_item_emb = item_weight * id_emb
            # weighted_text_emb = text_weight * emb
            ...
            weights = torch.stack([
                torch.sum(self.weight_emb * emb, dim=-1, keepdim=True) for emb in embeddings
            ], dim=-1)  # shape: (batch_size, 1, num_embeddings)
            
            # 使用 softmax 归一化
            normalized_weights = torch.softmax(weights.clamp(max=10), dim=-1)
            
            # 加权融合
            fused_emb = sum(w * emb for w, emb in zip(normalized_weights.unbind(dim=-1), embeddings))
            return fused_emb
